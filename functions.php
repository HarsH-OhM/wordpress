...
function internship_custom_rest(){
	register_rest_field('posts','message', array(
		'get_callback' => function(){
			return 'Hello InstaDukan';     //it will show this data in posts endpoint in json form ;check on postman
			
		}
	));
}

add_action('rest_api_init', 'internship_custom_rest');
...
function my_register_route() {
    register_rest_route( 'my-route', 'my-phrase', array(    //it will show this value to this path or created endpoint
                    'methods' => 'GET',                      //http://localhost/wordpressnew/wordpress/wp-json/my-route/my-phrase
...
function custom_phrase() {
    return rest_ensure_response( "custom_verified:boolean value(true:false)" );
}
...
function tutsplus_add_user_meta( $content ) {
    ...
    if ( 1 === get_the_ID() ) {
        add_user_meta( 1, 'custom_verified', 'true' );
    }
 
    return $content;
 
}    //here i have added this meta data tu user which is added to the database. u can check it in wp-usermata
